...
# Function to get sentence vector using GloVe embeddings
...
glove_vector_size = 100  # Adjust according to your GloVe model
glove_descriptions_to_match = [get_sentence_vector(tokens, glove_model, glove_vector_size) for tokens in preprocessed_descriptions_to_match]
glove_target_descriptions = [get_sentence_vector(tokens, glove_model, glove_vector_size) for tokens in preprocessed_target_descriptions]

# Convert lists to numpy arrays for cosine similarity calculation
glove_descriptions_to_match = np.array(glove_descriptions_to_match)
glove_target_descriptions = np.array(glove_target_descriptions)

# Compute cosine similarities between descriptions to match and target descriptions
cosine_similarities = cosine_similarity(glove_descriptions_to_match, glove_target_descriptions)
